import type { Exercise, ExerciseType, Lesson, LocalizedText, UserLevel, Word } from "../../src/types";
...
function toPlainText(v: LocalizedText | undefined): string {
  if (!v) return "";
  if (typeof v === "string") return v;
  const obj = v as Record<string, unknown>;
  for (const key of ["uk", "ru", "sk", "en"]) {
    const value = obj[key];
...
const VALID_EXERCISE_TYPES = new Set([
  "multiple_choice_translation","multiple_choice_context","choose_response","reverse_translation","audio_choice","match_pairs",
  "true_false","fill_blank","sentence_ordering","typing","mistake_review","single_choice","multiple_select","true_false_list",
  "dropdown_blank","cloze_text","word_bank","drag_to_blank","drag_to_category","matching","image_match","sentence_builder",
  "sentence_order","dialogue_order","dialogue_choose_reply","branching_dialogue","find_error","correct_error","transformation",
  "listen_choice","listen_true_false","listen_fill","dictation","reading_comprehension","meaning_in_context","natural_phrase",
  "tone","register","hidden_meaning","collocation","real_document","real_message","real_menu","real_schedule"
]);

const PASSTHROUGH = [
  "instruction","skill","prompt","text","statement","statements","sentence","sentenceParts","textParts","wordBank","items",
  "draggable","correct","categories","pairs","tokens","correctSentence","correctOrder","lines","dialogue","nodes","startNode",
  "successMessage","errorToken","correctToken","source","audioRef","imageRef","context","target","situation","phrase","document",
  "message","menuData","schedule","questions","acceptedAnswers","hint","ignoreCase","ignoreTerminalPunctuation","extraWords","displaySentence"
] as const;
...
function validateExercise(raw: Record<string, unknown>, lessonId: string, idx: number): Exercise {
  ...
  if (type === "multiple_choice" || type === "multiple_choice_reading" || type === "mini_situation") {
    type = "multiple_choice_translation";
  }
  if (!VALID_EXERCISE_TYPES.has(type)) throw new Error(`Вправа #${idx + 1}: невідомий тип "${type}"`);

  const extra: Record<string, unknown> = {};
  for (const key of PASSTHROUGH) if (raw[key] !== undefined) extra[key] = raw[key];
  ...
  return {
    ...
    correctAnswer: raw.correctAnswer === undefined ? undefined
      : Array.isArray(raw.correctAnswer) ? raw.correctAnswer as string[]
      : typeof raw.correctAnswer === "boolean" ? raw.correctAnswer
...
export function normalizeLessonPayload(raw: unknown): Lesson {
  ...
  if (!r || typeof r !== "object") throw new Error("Некоректний урок");
  ...
  const words = Array.isArray(r.words)
    ? r.words.map((w, i) => validateWord(w as Record<string, unknown>, id, i, level, topic))
    : [];
  const exercises = Array.isArray(r.exercises)
    ? r.exercises.map((e, i) => validateExercise(e as Record<string, unknown>, id, i))
    : [];
